...
module Parsers
  ...
  class Parser
    ...
    def parse_program
      statements = []
      while @cur_token.token_type != Tokens::EOF
    ...
    def parse_expression(precedence)
      prefix = @prefix_parsers[@cur_token.token_type]
      if prefix.nil?
        no_prefix_parse_error(@cur_token.token_type)
        return nil
      end
      left = prefix.call
      while !peek_token_is?(Tokens::SEMICOLON) && precedence < peek_precedence
        infix = @infix_parsers[@peek_token.token_type]
    ...
    def peek_error(token_type)
      @errors << "Expected next token to be #{token_type}, got #{@peek_token.token_type} instead"
    end
    ...
    def peek_precedence
      find_precedence(@peek_token.token_type)
    end
    ...
    def cur_precedence
      find_precedence(@cur_token.token_type)
    end
  end
end
